...
class Jpop extends Chariot.Command {
    constructor() {
        super();
        ...
        this.cooldown = 5;
        this.help = {
            message: 'Tell Hinoiri to come join your voice channel and start playing J-Pop music!',
            usage: 'jpop',
            example: ['jpop'],
            inline: true
        }
    ...
    async execute(message, args, chariot) {
        ...
        chariot._liveConnections.set(voiceChannel.guild.id, 'jpop');

        message.channel.createEmbed(new Chariot.RichEmbed()
            .setColor(0xFF9185)
            .setTitle('🎶 Now Playing')
            .setThumbnail(chariot._currentlyPlaying.jpop.cover)
            .setTimestamp(chariot._currentlyPlaying.jpop.started)
            .setFooter('Started playing')
            .setDescription(`**${chariot._currentlyPlaying.jpop.title}** by **${chariot._currentlyPlaying.jpop.artist}**`)
            .addField('Duration', chariot._currentlyPlaying.jpop.duration)
        );

        HinoiriHelper.lavalink.play(chariot, voiceChannel, 'https://listen.moe/stream');
